...
bool Map::is_solid(glm::vec3 position, float *penetration_x, float *penetration_y, int map_id)
{
    ...
    *penetration_y = 0;
    ...
    if(map_id == 1){
        if (tile == 0 || tile == 20 || tile == 21 || tile == 22 || tile == 23 || tile == 26 || tile == 27 || tile == 28 || tile == 29 || tile == 33 || tile == 34 || tile == 35) return false;
    }else if(map_id == 2){
        std::cout << "yes" << std::endl;
        if(tile == 48 || tile == 49) return false;
    }
